    ...
    def generate_cl_data(self, flip_rate, max_option_list, seed):
        ...
        if flip_rate > 0:
            for question, concept, score, option, answer in zip(self.question, self.concept, self.score, self.option, self.answer):
                ...
                pos_score = deepcopy(score)
                pos_option = deepcopy(option)
                neg_score = deepcopy(score)
                neg_option = deepcopy(option)
                    ...
                    if concept[t] in target_info:
                        if rand.random() < flip_rate:
                            ...
                            if target_info[concept[t]]["score"] == 1:
                                # pos: 若当前题正确率 <= 基准，则翻转成 1
                                # neg: 若当前题正确率 > 基准，则翻转成 0
                                ...
                                # 如果 base题是0
                                # pos: 若当前题正确率 > base则翻转成0
                                # neg: 若当前题正确率 <= base则翻转成1
                                if self.correct_rate[question[t]] > target_correct_rate:
                                    pos_score[t] = 0
            ...
            # flip_rate=0时, 不做任何翻转
            pos_scores = deepcopy(self.score)
            pos_options = deepcopy(self.option)
            neg_scores = deepcopy(self.score)
            neg_options = deepcopy(self.option)

        return {
            "pos_score": pos_scores,
            "pos_option": pos_options,
            "neg_score": neg_scores,
            "neg_option": neg_options
        }
...
def pad_collate(batch):
    ...
    return (
        user_pad, question_pad, concept_pad, score_pad, option_pad, answer_pad, unchosen_pad,
